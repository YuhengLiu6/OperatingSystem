...
__device__ __managed__ int storage_index = 0;
// __device__  __managed__ u32 storage_swaptoPM;
...
//swap table---------------------
__device__ __managed__ u32 swaptable[5120];
// page table
...
__global__ void mykernel(int input_size) {
  ...
  VirtualMemory vm;
  // vm_init(&vm, data, storage,swaptable, pt, &pagefault_num, &storage_index, &storage_swaptoPM, PAGE_SIZE,
  //         INVERT_PAGE_TABLE_SIZE, PHYSICAL_MEM_SIZE, STORAGE_SIZE,
  //         PHYSICAL_MEM_SIZE / PAGE_SIZE);

  vm_init(&vm, data, storage, swaptable, pt, &pagefault_num, &storage_index, PAGE_SIZE,
          INVERT_PAGE_TABLE_SIZE, PHYSICAL_MEM_SIZE, STORAGE_SIZE,
          PHYSICAL_MEM_SIZE / PAGE_SIZE);
